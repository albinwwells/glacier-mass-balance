...
class fullGlacier(glacier):
    ...
    def climMassBalance(self, smoothVel=0):
        ...
        qy is multiplied by -1 since we need positive direction to be south in np.gradient
        '''

        # df_vx = pandas.DataFrame(sourceList[2] * glacierOutlineFileRead)
        ...
        qxRaster = sourceList[1] * self.vCol * np.array(sourceList[2])
        qyRaster = sourceList[1] * self.vCol * np.array(sourceList[3]) * -1
