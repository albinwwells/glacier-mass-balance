...
def dynamicSmoothing(z, window_weight, pixel_size, f):
    ...
    for r in range(z.shape[0]):
        for c in range(z.shape[1]):
            # half of window size (must be at least 2 * f pixels)
            half_size = int(f * max(window_weight[r, c], 4 * pixel_size) / pixel_size)
