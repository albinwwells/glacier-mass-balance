from Modules.glacierClass import glacier
import numpy as np
import sys

# h_data='MillanThicknessFlowlineCorr' is only added for glac_name='Other'. Need to add it for specific glaciers

def glacierInfo(glac_name, res, avgDensity, elevationBinWidth, vCol, dem_data, h_data, v_data, shp_data):
    # -------------------------------------------------GULKANA---------------------------------------------------------#
    if glac_name == 'Gulkana':
        ...
        gulkana = glacier(name, type, shape, demFile, hFile, dhdtFile,
    ...
    # -------------------------------------------------WOLVERINE-------------------------------------------------------#
        ...
        data_var = abs(np.array([np.array(data_min) - np.array(data[0]), np.array(data_max) - np.array(data[0])]))
        wolverine = glacier(name, type, shape, demFile, hFile, dhdtFile,
                            crs, res, avgDensity, elevationBinWidth, data, data_var, vxFile, vyFile, vel_cor, vCol)
        return wolverine
    # ---------------------------------------------------EKLUTNA-------------------------------------------------------#
        ...
        data_var = None
        eklutna = glacier(name, type, shape, demFile, hFile, dhdtFile,
                          crs, res, avgDensity, elevationBinWidth, data, data_var, vxFile, vyFile, vel_cor, vCol)
        return eklutna
    # -------------------------------------------------LEMON CREEK-----------------------------------------------------#
        ...
        data_var = abs(np.array([np.array(data_min) - np.array(data[0]), np.array(data_max) - np.array(data[0])]))
        lemonCreek = glacier(name, type, shape, demFile, hFile, dhdtFile,
                             crs, res, avgDensity, elevationBinWidth, data, data_var, vxFile, vyFile, vel_cor, vCol)
        return lemonCreek
    # ---------------------------------------------------RAINBOW-------------------------------------------------------#
        ...
        data = None
        data_var = None
        rainbow = glacier(name, type, shape, demFile, hFile, dhdtFile,
                          crs, res, avgDensity, elevationBinWidth, data, data_var, vxFile, vyFile, vel_cor, vCol)
        return rainbow
    # ------------------------------------------------SOUTH CASCADE----------------------------------------------------#
        ...
        data = None
        data_var = None
        southCascade = glacier(name, type, shape, demFile, hFile, dhdtFile,
                               crs, res, avgDensity, elevationBinWidth, data, data_var, vxFile, vyFile, vel_cor, vCol)
        return  southCascade
    # ---------------------------------------------------CONRAD--------------------------------------------------------#
        ...
        data_var = None
        # WATCH OUT FOR VELOCITIES! MAKE SURE ITS CORRECT (SLOPE FROM DEM IS MESSING IT UP)
        conrad = glacier(name, type, shape, demFile, hFile, dhdtFile,
                         crs, res, avgDensity, elevationBinWidth, data, data_var, vxFile, vyFile, vel_cor, vCol)
        return conrad
    # -----------------------------------------------ILLECILLEWAET-----------------------------------------------------#
        ...
        data_var = None
        illecillewaet = glacier(name, type, shape, demFile, hFile, dhdtFile,
                                crs, res, avgDensity, elevationBinWidth, data, data_var, vxFile, vyFile, vel_cor, vCol)
    ...
    # ---------------------------------------------------NORDIC--------------------------------------------------------#
        ...
        data_var = None
        nordic = glacier(name, type, shape, demFile, hFile, dhdtFile,
                         crs, res, avgDensity, elevationBinWidth, data, data_var, vxFile, vyFile, vel_cor, vCol)
        return nordic
    # --------------------------------------------------ZILLMER--------------------------------------------------------#
        ...
        data_var = None
        # WATCH OUT FOR VELOCITIES! MAKE SURE ITS CORRECT (SLOPE FROM DEM IS MESSING IT UP)
        zillmer = glacier(name, type, shape, demFile, hFile, dhdtFile,
                          crs, res, avgDensity, elevationBinWidth, data, data_var, vxFile, vyFile, vel_cor, vCol)
        return zillmer
    # -------------------------------------------------RIKHA SAMBA-----------------------------------------------------#
        ...
        data_var = None
        rikhaSamba = glacier(name, type, shape, demFile, hFile, dhdtFile,
                             crs, res, avgDensity, elevationBinWidth, data, data_var, vxFile, vyFile, vel_cor, vCol)
        return rikhaSamba
    # ---------------------------------------------ALL OTHER GLACIERS--------------------------------------------------#
    else:
        ...
        type = 'Climatic'                               # Mass balance type
        if shp_data == 'RGI':
            shape = '01_rgi60_Alaska-2.shp'                 # shapefile from RGI
        elif shp_data == 'USGS':
            shape = 'Gulkana_USGS_2021.shp'
        dem_data = ''                                   # placeholder: get RGI ID, then update this in main_vel_comp
        ...
        if h_data == 'FarinottiThickness':
            ...
            hFile = ''
        elif h_data == 'OGGM_Thickness':
            hFile = ''
        elif h_data == 'FarinottiThicknessFlowlineCorr':
            hFile = ''
        elif h_data == 'MillanThicknessFlowlineCorr':
            hFile = ''
        elif h_data == 'OGGMThicknessFlowlineCorr':
            hFile = ''
        if v_data == 'ITS_LIVE_20yrComposite':
        ...
        data = None
        data_var = None
        otherGlacier = glacier(name, type, shape, dem_data, hFile, dhdtFile, crs, res, avgDensity,
                               elevationBinWidth, data, data_var, vxFile, vyFile, vel_cor, vCol)

        return otherGlacier
    # -----------------------------------------------------------------------------------------------------------------#

def glacierVelUncertainty(vel_selection, glacierCalcObject):
    subfolder = 'Velocity Error Files/'
    if vel_selection == 'ITS_LIVE_2017-2018':
        vx_err = subfolder + 'ITS_LIVE_vx_err_2017_2018_composite.tif'
        vy_err = subfolder + 'ITS_LIVE_vy_err_2017_2018_composite.tif'
    elif vel_selection == 'ITS_LIVE_2018':
        vx_err = subfolder + 'ITS_LIVE_vx_err_2018.tif'
        vy_err = subfolder + 'ITS_LIVE_vy_err_2018.tif'
    elif vel_selection == 'MillanVelocity_2017-2018':
        vx_err, vy_err = glacierCalcObject.MillanVelocityErrRGI1_2()
    elif vel_selection == 'RETREAT_2017-2018':
        # vx_err = 'Velocity Error Files/' + 'RETREAT_vx_std_2017-2018_composite.tif'
        # vy_err = 'Velocity Error Files/' + 'RETREAT_vy_std_2017-2018_composite.tif'
        vx_err = 'Velocity Error Files/' + 'RETREAT_vy_sterr_2017-2018_composite.tif'
        vy_err = 'Velocity Error Files/' + 'RETREAT_vy_sterr_2017-2018_composite.tif'
    elif vel_selection == 'RETREAT_2018':
        # vx_err = 'Velocity Error Files/' + 'RETREAT_vx_std_2018.tif'
        # vy_err = 'Velocity Error Files/' + 'RETREAT_vy_std_2018.tif'
        vx_err = 'Velocity Error Files/' + 'RETREAT_vx_sterr_2018.tif'
        vy_err = 'Velocity Error Files/' + 'RETREAT_vy_sterr_2018.tif'
    else:
        vx_err = None
        vy_err = None
