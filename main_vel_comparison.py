...
c3 = 'Aspect-Directed'      # this uses velocity magnitude but the direction is based entirely on the aspect
...
if glac[0] == 'All Other Glaciers':
    ...
    lb5 = Listbox(t3, height=3, exportselection=FALSE, justify='center')
    ...
    lb5.pack()
    t3.bind('<Return>', handler)
    t3.bind('<Double-1>', handler)
    b3 = Button(command=handler, text='SELECT', fg='black')
    b3.pack(side=BOTTOM)
    w = 350  # tk window width
    h = 80  # tk window height
    ...
    # Add GUI for Uncertainty - do we want to calculate it or not
    def handler(event=None):
        global vel_uncertainty_yn
        vel_uncertainty_yn = lb6.curselection()
        vel_uncertainty_yn = [lb6.get(int(x)) for x in vel_uncertainty_yn]
        t4.destroy()

    t4 = Tk()
    t4.title('Plot Velocity Uncertainty?')
    ...
    lb6.select_set(1)  # default beginning selection in GUI
    lb6.configure(justify=CENTER)
    lb6.pack()
    t4.bind('<Return>', handler)
    t4.bind('<Double-1>', handler)
    b4 = Button(command=handler, text='SELECT', fg='black')
    b4.pack(side=BOTTOM)
...
for glacier in range(number_of_glac):
    ...
    for j in range(number_of_vel):
        ...
        if glacierCalc.name == 'Other':
            ...
            if thick_select[0] == 'MillanThickness':
            ...
            elif thick_select[0] == 'OGGM_Thickness':
                ...
                glacierCalc.h = oggm_filepath + '/thickness_RGI60-01.' + glacierCalc.name + '.tif'
        ...
        if j == 0:
            slope_array_dem = glacierCalc.glacierAttributes(glacierCalc.dem[:-4] + '_clip.tif', 'slope_degrees')
            slope_map_title = glacierCalc.name + ' Slope Map'
            plotData(slope_array_dem, 'Slope (degrees)', 'RdYlGn_r', slope_map_title)
            aspect_array_dem = glacierCalc.glacierAttributes(glacierCalc.dem[:-4] + '_clip.tif', 'aspect')
            aspect_map_title = glacierCalc.name + ' Aspect Map'
            plotData(aspect_array_dem, 'Aspect (degrees from N)', 'twilight', aspect_map_title, 12)

        # ---------------------------------------GLACIER VELOCITY UNCERTAINTY--------------------------------------#
        if vel_uncertainty_yn[0] == 'Yes':
            print('Yes')
            # if vel_select[j] == 'MillanVelocity_2017-2018':  # for overlapping glaciers
            #     glacierCalc.vx, glacierCalc.vy = glacierCalc.MillanVelocityRGI1_2()

            # if vel_select[j] == 'ITS_LIVE_20yrComposite':
            #     # filepath
            # elif vel_select[j] == 'ITS_LIVE_2017-2018':
            #     # filepath
            # elif vel_select[j] == 'MillanVelocity_2017-2018':
            #     uncert_filepath = os.path.abspath(os.path.join(os.getcwd(), '../../../../../Volumes/LaCie' \
            #                                                                 '/Glacier Data Sets 2-18/Full Data Sets' \
            #                                                                 '/RGI Region 1 and 2/Farinotti Thickness and DEMs' \
            #                                                                 '/results_model_3_thickness/RGI60-01'))
            #     glacierCalc.h = uncert_filepath + '/thickness_RGI60-01.' + glacierCalc.name + '.tif'
            # elif vel_select[j] == 'RETREAT_2017-2018':
            #     # filepath
        ...
        vel_x_cor, vel_y_cor = glacierCalc.velAspectCorrection(aspect_array_dem, smoothvxArray, smoothvyArray, 90)
        vel_total_cor = np.power((np.square(vel_x_cor) + np.square(vel_y_cor)), 0.5)
        corrected_vels_quiver = velPlot(vel_x_cor, vel_y_cor, vel_total_cor, area, threshold=1.0)  # plot velocity arrow
        aspect_cor_title = vel_select[j] + ' Aspect-Corrected Map'
        plotData(vel_total_cor, 'Velocity (m/a)', 'BrBG', aspect_cor_title, cluster=None, quiver=corrected_vels_quiver)

        vel_x_aspect, vel_y_aspect = glacierCalc.velAspectDirection(aspect_array_dem, smoothvArray)
        vel_total_aspect = np.power((np.square(vel_x_aspect) + np.square(vel_y_aspect)), 0.5)
        aspect_vels_quiver = velPlot(vel_x_aspect, vel_y_aspect, vel_total_aspect, area, threshold=1.0)
        aspect_cor_title2 = vel_select[j] + ' Aspect-Directed Map'
        plotData(vel_total_aspect, 'Velocity (m/a)', 'BrBG', aspect_cor_title2, cluster=None, quiver=aspect_vels_quiver)
        # ---------------------------------------------------------------------------------------------------------#
        if filtered_vel[0] == 'Aspect-Corrected':
            divQRaster = ((np.gradient(vel_x_cor, glacierCalc.res)[1] * smoothhArray) +
                          (np.gradient(smoothhArray, glacierCalc.res)[1] * vel_x_cor) +
                          (np.gradient(vel_y_cor * -1, glacierCalc.res)[0] * smoothhArray) +
                          (np.gradient(smoothhArray * -1, glacierCalc.res)[0] * vel_y_cor)) * glacierCalc.vCol
        elif filtered_vel[0] == 'Aspect-Directed':
            divQRaster = ((np.gradient(vel_x_aspect, glacierCalc.res)[1] * smoothhArray) +
                          (np.gradient(smoothhArray, glacierCalc.res)[1] * vel_x_aspect) +
                          (np.gradient(vel_y_aspect * -1, glacierCalc.res)[0] * smoothhArray) +
                          (np.gradient(smoothhArray * -1, glacierCalc.res)[0] * vel_y_aspect)) * glacierCalc.vCol
        else:
        ...
        vel_dict['aspect_cor_title%s' % j] = aspect_cor_title
        vel_dict['aspect_cor_title2%s' % j] = aspect_cor_title2
        if glacierCalc.data is not None:
    ...
    if skip == False:
        ...
        if create_pdf == True:
            ...
            pdf.cell(epw, 5, 'The following calculations were conducted with velocity products that are based on ' +
                     filtered_vel[0] + ' data:')
            pdf.ln(3)
            pdf.cell(epw, 5, '"Aspect-Corrected" removes velocities that oppose the aspect,' +
                     ' "Aspect-Directed" used velocity magnitudes but applies aspect direction')
            ...
            for i in range(number_of_vel):  # add aspect-corrected velocity plots to pdf
                pdf.ln(2 * th)
                fig5 = vel_dict.get('aspect_cor_title%s' % i).replace(' ', '_') + '.png'
                pdf.image('Figures/' + fig5, x=None, y=None, w=epw)
                os.remove('Figures/' + fig5)

            for i in range(number_of_vel):  # add aspect-corrected velocity plots to pdf
                pdf.ln(2 * th)
                fig6 = vel_dict.get('aspect_cor_title2%s' % i).replace(' ', '_') + '.png'
                pdf.image('Figures/' + fig6, x=None, y=None, w=epw)
            ...
            if filtered_vel[0] == 'Original (smoothed)':
                ...
                pdfName = glacierCalc.name + '_' + str(glacierCalc.res) + 'mRes_' + str(glacierCalc.bin) + 'mBin_' + \
                          time_span + 'dhdt_' + str(raw_data_smooth_factor) + str(divQ_smooth_factor) + 'smoothing_' + \
                          thick_select[0] + '_' + str(vel_select[0:number_of_vel]) + filtered_vel[0] + '.pdf'
