...
from Modules.dataPlots import plotData, plotData3, plotData6, velPlot
...
for glacier in range(number_of_glac):
    ...
    for j in range(number_of_vel):
        ...
        if glacierCalc.name == 'Other':
            ...
            glacierCalc.data, glacierCalc.dataVariability = glacierCalc.getReferenceData(time_span)
        ...
        # filter out glaciers with small areas
        skip = False
        if area / 1000000 < 1:
            skip = True
        ...
        raw_data_smooth_factor = 4
        divQ_smooth_factor = 1
        ...
        divQRaster = (dynamicSmoothing((np.gradient(smoothvxArray, glacierCalc.res)[1] * smoothhArray), hArray,
                                        glacierCalc.res, 0) +
                       dynamicSmoothing((np.gradient(smoothhArray, glacierCalc.res)[1] * smoothvxArray), hArray,
                                        glacierCalc.res, 0) +
                       dynamicSmoothing((np.gradient(smoothvyArray * -1, glacierCalc.res)[0] * smoothhArray), hArray,
                                        glacierCalc.res, 0) +
                       dynamicSmoothing((np.gradient(smoothhArray * -1, glacierCalc.res)[0] * smoothvyArray), hArray,
                                        glacierCalc.res, 0)) * glacierCalc.vCol
        ...
        # TO DO: compare MB to glaciologic measurements for each velocity product
        if glacierCalc.data is not None:
            ...
            for i in range(len(glacierCalc.data[1])):
                ...
                weighted_mean = \
                    np.mean([binnedClimaticMassBalanceSmooth[indexVal - 1] * binAreasList[indexVal - 1] / areaAdjacent,
                             binnedClimaticMassBalanceSmooth[indexVal] * binAreasList[indexVal] / areaAdjacent,
                             binnedClimaticMassBalanceSmooth[indexVal + 1] * binAreasList[indexVal + 1] / areaAdjacent])
            ...
            # let's only consider bins from the min to max from total mass balance
            validIndexes = list(range(binTotalChange.tolist().index(min(binTotalChange)),
                                      binTotalChange.tolist().index(max(binTotalChange))))
    ...
    if skip == False:
        ...
        fig2_3Quiver = None
        extraPlotTitle = glacierCalc.name + ' Thickness, Elevation, Total MB Plots'
        plotData3(fig2_1Vals, fig2_1cbar, fig2_1Color, fig2_1Title,
                  fig2_2Vals, fig2_2cbar, fig2_2Color, fig2_2Title,
                  fig2_3Vals, fig2_3cbar, fig2_3Color, fig2_3Title, extraPlotTitle, glacierCalc.res,
        ...
        if create_pdf == True:
            ...
            if glacierCalc.data is not None:
                ...
                pdf.ln(2 * th)
                pdf.set_font('Times', 'B', 12)
                pdf.cell(epw, 5, 'Error Metrics: to WGMS Stake Points', ln=1, align='C')
                pdf.ln(0.5)
                col_width = epw / 5
                pdf.set_font('Times', 'B', 9)
                ...
                pdf.ln(2 * th)
                pdf.set_font('Times', 'B', 12)
                pdf.cell(epw, 5, 'Error Metrics: to WGMS Stake Point L.O.B.F.', ln=1, align='C')
                pdf.ln(0.5)
                col_width = epw / 4
                pdf.set_font('Times', 'B', 9)
                for row in Error_to_ref_lobf:
                    ...
                    pdf.ln(th)  # 2 * th is for double padding
                pdf.set_font('Times', '', 8)
                pdf.cell(epw, 5, '**Positive error means the calculated product predicts more mass loss per elevation'
                                 'than the stake measurements**')
            ...
            for i in range(number_of_vel):
                pdf.ln(2 * th)
                fig3 = vel_dict.get('smoothedVPlotTitle%s' % i).replace(' ', '_') + '.png'
                pdf.image('Figures/' + fig3, x=None, y=None, w=epw)
                os.remove('Figures/' + fig3)

            pdfName = glacierCalc.name + '_' + str(glacierCalc.res) + 'mRes_' + time_span + 'dhdt_' + \
                      str(raw_data_smooth_factor) + str(divQ_smooth_factor) + 'smoothing_' + thick_select[0] + '_' + \
                      str(vel_select[0:number_of_vel]) + '.pdf'
